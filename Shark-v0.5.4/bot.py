...
async def check_user(user_id: int, event) -> User:
    ...
    full_name = ''
    # if name.__dict__["first_name"]:
    if hasattr(name,'first_name'):
        full_name = name.first_name
    if hasattr(name,"last_name") and name.last_name:
...
async def messages_handler(event: events.newmessage.NewMessage.Event):
    ...
    # /hb red envelope
        ...
        # make a uniqe id
        try:
            _id = max(envelopes, key=lambda e:e.id).id + 1
        except ValueError:
            # list is empty
            _id = 1
        # add new envelope
...
async def admin_message_handler(event):
    ...
    # /sf command to decrease money of all users  
        ...
        if target == "all":
            ...
            # deposit_amount = database.get_deposit(target)
            ...
            if target_user.balance < -amount:
                # print('需要删除存款', target_user.balance + amount)
                # database.make_deposit(target_user,target_user.balance + amount)
                database.add_investors_amount(target_user,target_user.balance + amount)
                # deposit_amount = database.get_deposit(sender_id)
                # print('删除后的存款',deposit_amount)
            
            # target_user = database.get_user(int(target))
            # print('剩余金额',target_user.balance)
        # await event.reply(texts.delete_money_status.format(amount=separator(amount), user=target) )
        await event.reply("成功删除金额，删除金额:"+ str(amount)+"，目标用户:"+ str(target) )
    ...
    # /settings change setting
        ...
        # print('/scz')
        pattern = re.match(r'\/scz (?P<zu>\d+)$', text,re.I)

        if (not pattern):
            return await event.reply(texts.del_money_wrong_format)

        zu = int(pattern.group('zu'))
        
        database.dete_balance_all(zu)
        await event.reply("删除成功，本次删除组数："+str(zu))
