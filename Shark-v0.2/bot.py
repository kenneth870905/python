...
async def admin_message_handler(event):
    ...
    # /zengjia add money to users by admin
        ...
        print('用户信息金额',balance)
        amount2 = balance[0: len(balance)+ (amount * 4) ]
        print('amount2',amount2)
        # print(target_user,target_user.balance - int(amount2))
        print('扣除金额', target_user.balance - int(amount2) )
        print('扣除金额2', 0 - (target_user.balance - int(amount2) ) )
        print('扣除后金额',int(balance)  + (0 - (target_user.balance - int(amount2) ))  )
        # database.add_balance(target_user, 0 - target_user.balance - int(amount2) )

        await event.reply(texts.delete_money_status.format(amount=separator(amount), user=target) )
        
        # print(target_user.balance)
        # print('长度',len(balance)+amount)
        # print(balance[0: len(balance)+ (amount * 4)  ])
...
async def callback_handler(event: events.callbackquery.CallbackQuery.Event):
    ...
    if query == 'balance':
        ...
        await event.answer(texts.sign_in_reward.format( sign_time=sign_time,balance=separator(user.balance), reward=separator(reward)), alert=True)
        ...
        if game_id in games:
            ...
            if game_type in signed_games:
                game = signed_games[game_type](game_id, database)
            else :
        ...
        _player_list = list(
            map(lambda x: texts.user_bet_stat.format(name=x.user.name, amount=separator(x.bet_amount)), game.players))
        _player_list = "\n".join(_player_list)
...
if __name__ == "__main__":
    ...
    bot.add_event_handler(users_private_handler, events.NewMessage(pattern=r'^[/!](ck|qk)'))
