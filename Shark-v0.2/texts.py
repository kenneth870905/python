...
# sign_in_reward = " 你将获得 {reward} 硬币作为奖励 "
sign_in_reward = """
签到成功
    系统赠送了您：${reward}
    当前总余额：$ {balance}
    梭哈一时爽，请理性游戏~
    每{sign_time}秒可再次签到领取
"""
...
# sign_in_high_balance_warning = "您不能使用大于{sign_limit_balance}的余额进行登录。 提醒。 由于资金充足，本次登录失败  "
...
withdraw_warning = "每次提款金额不得少于100,000美元。 提现金额为:   ${amount}"
...
""" % ad
    ...
    user_game_stats = "   {name} : {cards} , 总分 : {score} ({status})"
    ...
    user_settle_stats = "   {name} : {cards} , 总分 : {score} ({status}) {amount}"

    dealer_settle_stats = "   {cards} , 总分 : {score} ({status})"
